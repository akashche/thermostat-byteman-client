...
public abstract class ConfigEntryBase<T> implements ChartConfigEntry<T> {
    ...
    protected ConfigEntryBase(String type, String label, T defaultValue) {
        if (null == type) throw new ChartException("specified type is null");
        if (null == label) throw new ChartException("specified label is null");
        if (null == defaultValue) throw new ChartException("specified defaultValue is null");
        this.type = type;
        this.name = label;
        this.label = label;
